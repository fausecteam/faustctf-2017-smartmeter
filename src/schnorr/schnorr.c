...
static int hash_buffer(const u_char *data, u_int data_len, const EVP_MD*hasher, u_char **dest, u_int *dest_size) {
    int size = EVP_MD_size(hasher);
    *dest = calloc(size, 1);
    ...
    if (!mdctx) 
        return 1;
    int res = EVP_DigestInit_ex(mdctx, hasher, NULL);
    if (!res)
        goto clean;
    res = EVP_DigestUpdate(mdctx, data, data_len);
    if (!res)
        goto clean;
    res = EVP_DigestFinal_ex(mdctx, *dest, dest_size);
    if (!res)
        goto clean;
...
clean:
    ...
    res = !res; // openssl and openssh define their errors in opposite ways
    return res;
}
/*
    ...
    const u_char *id, u_int idlen)
{
	...
	SCHNORR_DEBUG_BUF((buffer_ptr(&b), buffer_len(&b),
	    "%s: hashblob", __func__));
	if (hash_buffer(buffer_ptr(&b), buffer_len(&b), EVP_sha256(), &digest, &digest_len) != 0) {
